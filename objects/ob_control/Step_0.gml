...
battler_turn=1 and card_hold=-1 and ob_main.cursor_hide=false {
	...
	if auto_attack_timer=0 and (ob_main.playing_tutorial=false or (ob_main.playing_tutorial=true and sc_tutorial_conditions(5,-1))) {
		if turn_num>2 {
			...
			card_space_id[auto_attack_slot+5].occupy_id.potential_x=card_space_id[auto_attack_slot+5].occupy_id.x and
			card_space_id[auto_attack_slot+5].occupy_id.potential_y=card_space_id[auto_attack_slot+5].occupy_id.y and
			...
			else if auto_attack_slot>3 { auto_attack_slot=0; }
		}
		else if ob_main.autoattack_toggle=false {
			first_turn_attack_warning=true;
			tooltip_timer=tooltip_timer_max;
		}
	}
